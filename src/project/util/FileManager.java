...
public class FileManager {
    ...
    private static final String COACHES_FILE      = "coaches.txt";
    ...
    private static final String TEAM_COACH_FILE   = "team_coach.txt";
    ...
    public static void saveCoaches(List<Coach> coaches) {
        try (PrintWriter pw = new PrintWriter(new FileWriter(COACHES_FILE))) {
            for (Coach c : coaches) {
                pw.println(c.getCoachId() + "|" + c.getName() + "|" + c.getAge() + "|" + c.getNationality() + "|" + c.getSpeciality());
            }
        } catch (IOException e) {
            System.out.println("[FileManager] Error saving coaches: " + e.getMessage());
        }
    }
    ...
    public static List<Coach> loadCoaches() {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(file))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                list.add(new Coach(name, age, nationality, speciality));
            }
        } catch (IOException e) {
            System.out.println("[FileManager] Error loading coaches: " + e.getMessage());
        }
        return list;
    }
    ...
    public static void saveTeamCoach(List<Team> teams) {
        try (PrintWriter pw = new PrintWriter(new FileWriter(TEAM_COACH_FILE))) {
            for (Team t : teams) {
                if (t.getCoach() != null) {
                    pw.println(t.getTeamId() + "|" + t.getCoach().getCoachId());
                }
            }
        } catch (IOException e) {
            System.out.println("[FileManager] Error saving team-coach: " + e.getMessage());
        }
    }

    public static void loadTeamCoach(TeamService teamService, CoachService coachService) {
        File file = new File(TEAM_COACH_FILE);
        if (!file.exists()) return;
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(file))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (t != null && c != null) t.addCoach(c);
            }
        } catch (IOException e) {
            System.out.println("[FileManager] Error loading team-coach: " + e.getMessage());
        }
    }
    ...
    public static void savePassword(String password) {
        try (PrintWriter pw = new PrintWriter(new FileWriter(PASSWORD_FILE))) {
            ...
            System.out.println("[FileManager] Error saving password: " + e.getMessage());
        }
    }
    ...
    public static String loadPassword() {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(file))) {
            ...
            System.out.println("[FileManager] Error loading password: " + e.getMessage());
            return null;
        }
    }
}